...
/*lenovo-sw, zhouwl, 2012-10-17, add for hall*/
...
#if defined(LENOVO_PROJECT_SEINE)||defined(LENOVO_PROJECT_PRADA)//lenovo jixu add
        msleep(1000);       //1s
#else
        msleep(2000);       //2s
#endif
...
//begin lenovo jixu add for fix lcd blurred screen when power on and system resume;
extern unsigned int bl_brightness;
extern int mt65xx_leds_brightness_set_custom(int type, int level);
//end lenovo jixu add for fix lcd blurred screen when power on and system resume;
#ifdef CONFIG_HAS_EARLYSUSPEND
...
//begin lenovo jixu remove for fix lcd blurred screen when power on and system resume;
//	is_early_suspended = FALSE;
//end lenovo jixu remove for fix lcd blurred screen when power on and system resume;	
//	DISP_SetBacklight(0);
#if 0
//	memset(fbVirAddr, 0, MTK_FB_SIZE);
    ...
    //begin lenovo jixu add for fix lcd blurred screen when power on and system resume;
	mt65xx_leds_brightness_set_custom(6, bl_brightness);
	//end lenovo jixu add for fix lcd blurred screen when power on and system resume;
